...
2. The correct  visual_type to use (BAR_CHART or PIE_CHART or FORECAST), with the required parameters. You have to return visual_type for each task otherwise don't return any task.
...
You will never return a visualization task with empty parameters and visual_type.
...
def get_plan(table_info, question):
...
class VisualizationTask(BaseModel):
    ...
    visual_type: Literal["BAR_CHART", "PIE_CHART", "FORECAST"] = Field(..., description = "Type of the visualization chart")
    ...
    def run(self):
        ...
        print(data)
        if self.visual_type == VisualizationType.BAR_CHART:
            fig = get_bar_chart(
                data=data, title=self.title, barmode=self.parameters.get("bar_mode")
            )
            return fig

        elif self.visual_type == VisualizationType.PIE_CHART:
            fig = get_pie_chart(data=data, title=self.title)
            return fig

        elif self.visual_type == VisualizationType.FORECAST: 
...
if len(st.session_state.user_input) > 0:
    ...
    with st.spinner("Generating query plan..."):
        start = timer()
        visualization_plan = get_plan(get_table_info(cur, "crypto_data"), user_input)
        end = timer()
